...
- **The market ties the simple mean of its agents and loses to precision weighting.** 200 paired trials scored against a *known* true probability, which no field study can do. Squared error to truth: market **0.03137**, simple mean 0.02960 (p = 0.54, indistinguishable), precision-weighted 0.01436 (**market 2.2x worse**, p < 0.0001), best agent chosen with hindsight 0.00272. See [Findings](#findings).
- **Experiment 2 rules out the matching rule.** The natural reading of Experiment 1 is that a limit order book aggregates by capital, since it needs a counterparty for every trade. So Experiment 2 built an LMSR venue, which always quotes and needs no counterparty. It lands in the same place, paired difference -0.00074 with a 95% interval of [-0.0045, +0.0024] and p = 0.68, which leaves the agents' balance sheets as the binding constraint.
...
|---|---|---|---|---|---|
| Best single agent (hindsight) | 0.00272 | **+0.02865** | [+0.023, +0.035] | <0.0001 | baseline wins |
| Simple mean | 0.02960 | +0.00177 | [-0.002, +0.006] | 0.54 | **indistinguishable** |
| Precision-weighted | 0.01436 | **+0.01701** | [+0.012, +0.023] | <0.0001 | baseline wins |
| Extremized log-odds | 0.02980 | +0.00157 | [-0.003, +0.007] | 0.54 | **indistinguishable** |

Three ablations rule out the easy explanations. Removing noise traders entirely changes nothing, 0.02796, with every comparison unchanged. Raising position limits five-fold changes nothing, 0.03159. Error by session length is flat from 300 seconds onward, so the market has converged and is not short of time.
...
The market is healthy while this happens, which is what makes it a result about markets rather than about a broken simulation: 2,370 trades per trial, two-sided 99.8% of the time, reliability 0.0076, conservation exact in every trial.

### Experiment 2: is it the mechanism?

An LMSR venue carries none of the constraints an order book imposes. It always quotes, it needs no counterparty, and it has no market maker anchoring price to its own reference. It reaches the same answer, p = 0.68, which isolates the balance sheet rather than the matching rule as the binding constraint. Its error is 0.03063 against the order book's 0.03137, a nominal lead that reverses the sign of the difference published before the open-order reserve and sits well inside the null either way. Full write-up in [experiments/information_aggregation/](experiments/information_aggregation/README.md).
